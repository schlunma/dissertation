...
\begin{figure}[t]
  ...
  \caption{Global and annual mean net \acf{TOA} radiation $N$ \vs{} global and
    annual mean near-surface air temperature $\Delta T$ for 150 years of a
    simulation with an abrupt quadrupling of the atmospheric \ac{CO2}
    concentration (4x\acs{CO2}) for the \acs{CMIP}5 multi-model mean (blue
    squares). To account for energy leakage and model drift, a linear fit of
    the corresponding pre-industrial control run is subtracted from the
    4x\acs{CO2} simulation. As given by \cref{eq:02:N_vs_delta_t}, the slope of
    the linear regression (black line) corresponds to the climate feedback
    parameter $\lambda$ and the y-intercept corresponds to the radiative
    forcing $F_\text{4x}$. These can be used to calculate the \ac{ECS} with
    the Gregory regression method following to \cref{eq:02:ecs}
    \autocite{Gregory2004}.}
...
(4x\acs{CO2}) \autocite{Gregory2004}. The basis of the definition of \ac{ECS}
...
steady state values for $F$ and $\lambda$ can be estimated from a 4x\acs{CO2}
...
4x\acs{CO2} run are plotted (see \cref{fig:02:gregory_regression}). To account
...
the 4x\acs{CO2} simulation beforehand \autocite{Andrews2012}. Since the
...
running 4x\acs{CO2} simulation with more than 1000 model years is considered.
...
4x\acs{CO2} instead of a 2x\acs{CO2} simulation. The factor of $2$ in the
...
the climate system but describes the transient response of an evolving climate.
\ac{TCR} is defined as the change in the annual mean \ac{GSAT} at the time of
\ac{CO2} doubling in a simulation with a \pct{1} per year increase of the
atmospheric \ac{CO2} concentration (1\%\acs{CO2}) averaged over a 20 year
period centered at the time of the doubling (years \range{61}{80} when the
first year is indexed with 1) \autocite{Bindoff2013}. To account for model
drift, the annual mean changes in \ac{GSAT} are calculated relative to a
corresponding pre-industrial control simulation smoothed with a linear fit that
considers 140 model years (length of the 1\%\acs{CO2} simulation).

- ocean heat uptake (TCR)

- residual heat -> 90\% go into ocean (Knutti et al. 2017)
