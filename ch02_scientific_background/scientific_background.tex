...
This \lcnamecref{ch:02:scientific_background} introduces the scientific
...
variable is usually given. Examples of such variables provided by a climate
...
external forcings, which are incorporated as boundary conditions into the
climate models. For example, the major driver of anthropogenic climate change,
the \ac{GHG} \ac{CO2}, is either included with prescribed atmospheric \ac{CO2}
concentrations (\emph{concentration-driven runs}) or prescribed \ac{CO2}
emissions (\emph{emission-driven runs}).

Many processes in the Earth system occur at spatial scales much smaller than
the size of a typical grid cell. Illustrative examples of this are clouds,
...
simulation of the period \range{1850}{2014} and the so-called \emph{\ac{DECK}}
...
corresponding \lcnamecrefs{ch:04:papers_ecs_tcr_assessment}.
...
\autocite{Hawkins2009, Hawkins2010}. \Cref{fig:02:sources_of_uncertainty} shows
...
might take. Examples of these are the \acs{SSP}-based experiments given by
...
geographical distribution of clouds. Examples of possible cloud feedback
...
of incoming solar radiation and also increases surface warming
...
atmosphere is used for that: In equilibrium, the absorbed incoming solar
irradiance (after accounting for reflection) is balanced with an outgoing
thermal irradiance $J_0$ that solely depends on the global mean surface
temperature $T_0$ following the Stefan-Boltzmann law
\begin{equation}
  J_0 = -\sigma T_0^4.
  \label{eq:02:stefan_boltzmann_law_eq}
\end{equation}
$\sigma \approx 5.67 \cdot 10^{-8} \unit{W m^{-2} K^{-4}}$ is the
Stefan-Boltzmann constant. To answer an external forcing $F$, the climate
system reacts with a response $R$ expressed by a change in the global mean
surface temperature $\Delta T$:
\begin{equation}
...
anthropogenic climate change, the \emph{Charney Report} from 1979
...
\begin{figure}[t]
  ...
  \caption[
    ...
    (\nxcotwo{4}) for the \acs{CMIP}5 \acl{MMM} (blue points). To account for
...
long-running \nxcotwo{4} simulation with more than 1000 model years is
...
\textcite{Sherwood2020} show that the \ac{ECS} is only about $6 \unit{\%}$
...
\lcnamecref{sec:02:climate_sensitivity} are by definition independent of carbon
...
\lcnamecref{sec:02:carbon_cycle} introduces the scientific background of the
...
\autocite{Arora2020}. Examples of photosynthesis (\ac{GPP}) are given by
...
are known to affect photosynthesis, such as the availability of light, \ac{CO2}
concentration, soil moisture, and temperature.
...
climate projections. This \lcnamecref{sec:02:reducing_uncertainties} introduces
...
of the work presented in the following \lcnamecrefs{ch:05:paper_ecs}.
...
\lcnamecref{sec:02:reducing_uncertainties}, one main issue of multi-model
...
Further techniques to reduce uncertainties in climate model projections with
...
previous \lcnamecref{subsec:02:emergent_constraints}, $y$ is a target variable
...
\lcnamecref{subsec:02:model_weighting}: Weighting schemes should address the
question \enquote{which climate model is adequate for predicting the target
...
However, a much weaker assumption is made in traditional weighting approaches
