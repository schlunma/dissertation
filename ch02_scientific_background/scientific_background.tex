...
unweighted \ac{MMM} and the multi-model range as best estimate and uncertainty
measure of this quantity \autocite{Collins2013}. This \emph{one model one vote}
or \emph{model democracy} approach basically assumes that all climate models
are independent, equally plausible, distributed around reality and that the
projected multi-model range is representative for the uncertainty in the
projected quantity \autocite{Knutti2017a}. However, since the \ac{CMIP}
...
\autocite{Knutti2013}, that models do not equally well represent the observed
past and present-day climate \autocite{Knutti2013} and that models might suffer
from common structural limitations like missing processes
\autocite{Knutti2017a}.
...
\end{equation}
for the predicted target variable $\hat{y}$ with intercept $a$ and slope $b$ is
used. Fitting this regression line with ordinary least squares includes
...
constrain the regression model, \eg{} by demanding a zero intercept ($a = 0$)
...
to other domains, like the global carbon cycle. For example, \textcite{Cox2013}
and \textcite{Wenzel2014} provide an emergent constraint on the terrestrial
climate-carbon feedback, \textcite{Wenzel2016} and \textcite{Winkler2019} on
the terrestrial concentration-carbon feedback. An extensive discussion on the
emergent constraint by \textcite{Wenzel2016} is given in \cref{sec:06:step_1}.
A further example of an emergent constraints on the carbon cycle focus on
primary production in the tropical oceans \autocite{Kwiatkowski2017}.
...
- recent studies: Brunner (T), Paul Young (Ozone), Lorenz?

- Lorenz (2018): reasoning for non-equal weighting, Collins 2013 as bad example

- Basic assumption: Models good in past -> good in future (PERFORMANCE)

- Models share code (INTERDEPENDENCE)

- Review by Abramowitz

- Theory: Knutti, p.1910


\subsection{\acl{MDER}}
\label{subsec:02:mder}

I do not like \ac{MDER}!
