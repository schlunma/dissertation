...
previous \lcnamecref{ch:04:papers_ecs_tcr_assessment} begs the question about
...
source of uncertainty for \ac{ECS} \commentcite{Boucher2013, Flato2013}{see
  also \cref{subsec:02:climate_feedbacks}}. Since all of the evaluated emergent
...
\begin{table}[!t]
  ...
  \caption[
    ...
    \lcnamecref{ch:05:paper_ecs}. Detailed descriptions of the variables used
...
In this \lcnamecref{ch:05:paper_ecs}, we first introduce the data (climate
...
ensembles (\cref{sec:05:comparison_of_emergent_constraints}). Finally, we
...
In this \lcnamecref{sec:05:comparison_of_emergent_constraints}, we describe and
...
changes in \acfp{SST}. Differences in the \ac{TLC} albedo account for more than
...
radiative fluxes over the period \range{2001}{2005}. In the original
...
\begin{figure}[p]
  ...
  \caption[
    ...
    \acs{CMIP}6 ensembles.
  ]{
    Emergent constraints BRI, COX, and LIP applied to the \acs{CMIP}5 (blue)
    and \acs{CMIP}6 ensembles (orange). (a), (c), (e) Emergent relationships
    ...
    for the different emergent constraints. \AdaptedFrom{Schlund2020a}.
  }
...
the period \range{1880}{2014}. Under the COX constraint we assess a $66
...
speed over the period \range{1980}{2005}. Our application of this emergent
...
publication does not specify \iac{ECS} range. For \acs{CMIP}6, the emergent
...
\autocite{Dee2011} over the period \range{1989}{1998} similar to the original
...
this constraint, we derive \iac{ECS} $66 \unit{\%}$ confidence range of
...
over the period \range{2003}{2005} as the reference dataset. We assess a $66
...
reference data, we use observed \acl{PR} data for the years \range{1986}{2005}
from GPCP \autocite{Adler2003}. We calculate \iac{ECS} $66 \unit{\%}$
...
0.62$). In their original publication, \textcite{Zhai2015} find \iac{ECS} range
...
\subsection{Constrained \acs{ECS} Ranges and Statistical Significance of the 11
...
\begin{figure}[t]
  ...
  \caption[
    ...
    constraints using the \acs{CMIP}5 and \acs{CMIP}6 ensembles.
  ]{
    ...
    ensembles. The bars correspond to the $66 \unit{\%}$ confidence intervals
...
\lcnamecrefs{sec:05:comparison_of_emergent_constraints}, most emergent
...
\lcnamecref{sec:05:discussion}, we discuss possible reasons for, and
...
In this \lcnamecref{ch:05:paper_ecs}, 11 different emergent constraints on
...
Our analysis makes several simplifying assumptions common to other studies,
