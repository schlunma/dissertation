...
\begin{table}[t]
  ...
  \begin{tabular}[t]{p{0.27\columnwidth} l p{0.11\columnwidth}
    ...
    \bottomrule
  \end{tabular}
  \caption{Process-oriented diagnostics (also called \emph{predictors} or
    ...
    change), the temporal mean over the specified time ranges is calculated
    for all variables. \AdaptedFrom{Schlund2020}.}
...
\ac{MMM} of future \ac{GPP} (\acs{MMM}), its rescaled version using
\cref{eq:06:y} (r\acs{MMM}), a linear \ac{LASSO} regression model using all
features as defined in \cref{tab:06:predictors} (\ac{LASSO}), a
single-predictor \ac{LASSO} model (\acs{LASSO}-1D) and a single-predictor
\ac{GBRT} model (\acs{GBRT}-1D). Both single-predictor models use the
historical \ac{GPP} as only feature. The predictive power (in terms of
\ac{RMSEP}) of each statistical model is assessed using the leave-one-model-out
\ac{CV} approach (see \cref{subsec:06:uncertainty_and_skill}). This allows us
to create an \ac{RMSEP} distribution for each statistical approach, where the
different points of the distributions refer to different training/prediction
dataset combinations generated by the leave-one-model-out \ac{CV} approach.
Thus, each distribution consists of seven points, one for each climate model.
...
\acp{RMSEP} compared to the \acs{MMM} and r\acs{MMM}. This also shows that the
...
\acs{MMM}. This ratio is almost constant over the whole globe, which is not
...
\unit{\GtCyr{}}$) as \acs{MMM} ($198 \unit{\GtCyr{}}$), which can be
