...
As argued in detail in the previous \lcnamecref{ch:05:paper_ecs},
...
\acs{RCP}8.5 scenario at the end of the \nth{21} century with a new
...
technique in combination with \iac{ML} algorithm. In the first step, we apply
...
\Cref{sec:06:data_and_methods} of this \lcnamecref{ch:06:paper_gpp} provides an
...
\subsection{Constraining the \texorpdfstring{\acs{CO2}}{CO\unichar{"2082}}
  Fertilization Effect (Step 1)}
\label{subsec:06:step_1}

In this \lcnamecref{subsec:06:step_1}, we present the observational emergent
...
doubling relative to pre-industrial conditions in \iac{1BGC} is constrained.
...
(Step 2) presented in the following \lcnamecref{subsec:06:step_2}. We note that
...
As already described in the previous \lcnamecref{subsec:06:step_1}, we analyze
...
\acf{MMM} and \iac{LASSO} model \autocite{Tibshirani1996}. The \ac{LASSO}
...
observed \ac{GPP} averaged over the years \range{1991}{2000} from the
...
\cref{eq:06:y}. The following two \lcnamecrefs{subsec:06:results_step2a} show
...
\cref{subsec:06:uncertainty_and_skill}). This allows us to create \iac{RMSEP}
...
In this \lcnamecref{subsubsec:06:results_step2a_obs}, we use the different
...
different regions. On the global domain, our approach predicts \iac{GPP} at the
...
All four patterns in \cref{fig:06:step2b_results} show very similar
...
To get further insights into the \ac{GBRT} algorithm, we compare it directly to
...
In this \lcnamecref{ch:06:paper_gpp}, a two-step approach to constrain the
...
constraint in future studies. In the second step, \iac{ML} approach is used to
...
(Step 2a) and $16 \unit{\%}$ (Step 2b); compared to a linear \ac{LASSO}
