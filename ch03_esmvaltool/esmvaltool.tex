...
high-sensitivity climate models exhibit higher values of $\Psi$ than
low-sensitivity models, an emergent relationship between \ac{ECS} and $\Psi$
can be established (\cref{fig:03:cox18:b}). With observations of $\Psi$,
\ac{ECS} can eventually be constrained to $\left( 2.8 \pm 0.6 \right) \unit{K}$
($66 \unit{\%}$ confidence range). More details on this including the
calculation of $\Psi$ are given in \cref{subsec:05:cox}.
